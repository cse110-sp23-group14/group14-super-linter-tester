...
function showResult(answer) {
  ...
  const interval = setInterval(() => {
    if (i < answerText.length) {
      answerElement.textContent += answerText[i];
      i++;
    } else {
      clearInterval(interval);
    }
  }, 100); // Adjust the delay (in milliseconds) between each character display

  setTimeout(() => {
    window.location.href = "result.html"; // Redirect to the results page after displaying the answer
  }, answerText.length * 100); // Adjust the total display time based on the length of the answer and the delay per character
}
*/
